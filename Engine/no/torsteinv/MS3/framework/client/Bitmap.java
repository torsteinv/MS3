	...
	public void render(Game game) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				float yy = ((y + n(game.zOffs,width) + n(game.yOffs,width)) * (h / Art.tile.height))
						% h;
				
				float xx = (float) ((x + (y * 1) + n(game.xOffs,w) + n(game.yOffs,w)) % w);
				// try {
